...
## Stage 23 — Handedness Feature Expansion & Rule-Based Movement Classifier

**Goal:** Fix two failing subsystems: poor hand detection accuracy (64.6% LOSO) and unstable GMM clustering that collapsed 65% of all cycles into a single "other" bucket.

### Root Cause Analysis

**Handedness (64.6% LOSO accuracy):**
The original 6 features were entirely gyro_y-centric (p95/p05 asymmetry, skewness, weighted mean, two cross-axis correlations). They missed the most stable chirality signal: **static wrist tilt**. When a person eats right-handed vs left-handed the fork rests at a different angle relative to gravity — `mean(acc_y)` and `mean(acc_z)` encode this directly, independent of motion noise. Also missing: the sign of the tilt–rotation coupling `corr(acc_y, gyro_y)` which flips between hands.

**Clustering (268 of 410 cycles in "other"):**
GMM is unsupervised — cluster identity shifts with random initialization, and the post-hoc `GMM_CLUSTER_LABEL_MAP` assignment in `config.py` is fragile. Because scoop and stab are *biomechanically defined* (not discovered), an unsupervised method can't reliably find them.

### Changes

**`handedness.py` — 6 → 13 features:**

| New feature | Why |
|---|---|
| `mean_acc_y`, `mean_acc_z` | Gravity projection encodes static wrist tilt, which differs per hand due to wrist anatomy |
| `skew_acc_y` | Asymmetry of the tilt trajectory shape |
| `corr_ay_gy` | Tilt–rotation coupling — sign flips between hands (chirality-sensitive) |
| `corr_ay_gz` | Same for yaw direction |
| `mean_gy`, `mean_gz` | Net rotation direction, more robust than just extremes |

**`movement_classifier.py` — GMM replaced with rule-based classifier:**

Rules applied in priority order:
1. **fragment** — `duration < 2.0 s` AND `jerk_ratio < 2.5` (short, weak motion)
2. **scoop** — lateral tilt dominates (`acc_y_range > acc_z_range × 0.8`), roll rotation dominates (`gy_std > gx_std`), peak jerk in mid-cycle (0.25–0.75)
3. **stab** — vertical range dominates (`acc_z_range > acc_y_range × 0.8`), peak jerk early (< 0.55)
4. **other** — everything else

`fit()` is a no-op (no training required). All thresholds live in `config.py` as `RULE_*` constants and can be tuned by inspecting `cluster_inspection.pdf`.

**`config.py`:** Removed `GMM_N_COMPONENTS` and `GMM_CLUSTER_LABEL_MAP`. Added 10 `RULE_*` threshold constants.

**`main.py`:** Simplified Stage 4c — removed min-cycle guard and GMM-specific cluster-map log.

### Expected Results

**Handedness:** Modest improvement in LOSO accuracy. The fundamental bottleneck remains the class imbalance (378 Right vs 32 Left cycles) — no feature set can overcome ~3 Left-hand training examples per LOSO fold. More Fork2 (left-hand) recordings are needed to meaningfully improve above ~75%.

**Clustering:** Deterministic and reproducible across runs (no random seed dependency). The "other" bucket should shrink significantly. Scoop and stab labels are now biomechanically grounded, making the Stage 6b bucketed regression meaningful. Tuning the `RULE_*` thresholds in `config.py` while inspecting `cluster_inspection.pdf` allows iterative calibration without retraining anything.

**Files:** `handedness.py`, `movement_classifier.py`, `config.py`, `main.py`
