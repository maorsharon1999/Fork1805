...
def _hand_from_filename(filename: str) -> Optional[str]:
    ...
    for prefix, hand in cfg.FORK_HAND_MAP.items():
        if lower.startswith(prefix.lower()):
            return hand
    # Bare "Fork_..." — hand cannot be determined from filename;
    # return None so these cycles are excluded from handedness training
    # and their hand is predicted by the classifier instead.
    if lower.startswith("fork_") or lower == "fork":
        logger.debug("Cannot determine hand from filename: %s", filename)
        return None
    return None
