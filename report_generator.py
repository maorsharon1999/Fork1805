...
_FIGURE_FILES = [
    ...
    ("Confusion Matrix - ET vs Control", "confusion_matrix.png"),
    ("ROC Curve", "roc_et_vs_control.png"),
    ("Severity Confusion Matrix - Local Score", "severity_cm_local.png"),
    ("Severity Confusion Matrix - Global Score", "severity_cm_global.png"),
...
def generate_report(
    ...
    sev_local:  Dict = metrics_dict.get("sev_local",  {})
    sev_global: Dict = metrics_dict.get("sev_global", {})

    if sev_local or sev_global:
        _section_title(pdf, "3b. ET Severity Classification (Mild / Moderate / Severe)")
        pdf.set_font("Helvetica", "", 10)
        pdf.multi_cell(
            0, 5.5,
            _safe_text(
        ...
        sev_headers = ["Score", "Class", "N Patients", "LOSO Accuracy"]
        sev_widths  = [40, 35, 35, 50]

        for sev_dict, score_label in [
            (sev_local,  "Local Score"),
            (sev_global, "Global Score"),
        ]:
            if not sev_dict:
                continue
            pdf.set_font("Helvetica", "B", 10)
            pdf.cell(0, 7, score_label, ln=True)
            pdf.ln(1)
            ...
            if rows:
                # Only show accuracy on the first row to avoid repetition
                for i in range(1, len(rows)):
                    rows[i][3] = ""
                _draw_table(pdf, sev_headers, rows, sev_widths)
            pdf.ln(4)
