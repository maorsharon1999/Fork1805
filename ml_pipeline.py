...
def _aggregate_segment_predictions(df, y_pred, target_col):
    # global_score is patient-level (identical for both hands) — group by patient only
    if target_col == "global_score":
        key = df["patient_id"].astype(str)
    else:
...
def run_regression(features_df, target_col="local_score") -> Dict[str, Dict[str, float]]:
    ...
    for name, estimator in base_models.items():
        ...
        for train_idx, test_idx in cv_splitter:
            ...
            if not _logged_reg_features:
                vt = fold_pipe.named_steps.get("var_thresh")
                rfe = fold_pipe.named_steps.get("rfe")
                if vt is not None and rfe is not None:
                    vt_cols = X.columns[vt.get_support()]
                    sel_cols = list(vt_cols[rfe.support_])
                    logger.info("RFE (1st fold) selected %d features: %s", len(sel_cols), sel_cols)
...
def compute_patient_metrics(
    ...
    df = pd.DataFrame({"patient_id": patient_ids, "y_true": y_true, "y_score": y_score})

    def _top3mean(x):
        vals = np.sort(x.values)
        return float(np.mean(vals[-min(3, len(vals)):]))

    agg_methods = {"median": "median", "mean": "mean", "top3mean": _top3mean}
    per_method: Dict[str, Dict[str, Any]] = {}

    for agg_name, agg_func in agg_methods.items():
        patient_df = df.groupby("patient_id").agg(
            y_true=("y_true", "first"),
            y_score=("y_score", agg_func),
        )
        ...
        best_idx = valid_idx[np.argmax(tpr[valid_idx])] if len(valid_idx) > 0 else np.argmax(specificities)
    ...
    logger.info("    Patient aggregation (N=%d):", per_method["median"]["N_patients"])
    for agg_name, m in per_method.items():
        logger.info("      %-10s  AUC=%.3f  Sens=%.3f  Spec=%.3f",
                    agg_name, m["AUC"], m["Sensitivity"], m["Specificity"])

    # Flatten with suffix; keep bare "AUC" / "N_patients" as median for back-compat
    combined: Dict[str, Any] = {}
    for agg_name, m in per_method.items():
        for k, v in m.items():
            combined[f"{k}_{agg_name}"] = v
    combined["AUC"] = per_method["median"]["AUC"]
    combined["N_patients"] = per_method["median"]["N_patients"]
    return combined
...
def run_classification(features_df) -> Dict[str, Dict[str, float]]:
    ...
    Runs twice:
      ...
      - all segments (no suffix) — comparison / baseline
    """
    from sklearn.pipeline import Pipeline
    from sklearn.feature_selection import RFE
    from sklearn.base import clone
    ...
    _datasets: List[tuple] = []
    if "movement_type" in features_df.columns:
        df_mo = features_df[features_df["movement_type"].isin(["scoop", "stab"])].copy()
        if len(df_mo) >= 5:
            _datasets.append(("_mo", df_mo))
        else:
            logger.warning("Motion-only filter: only %d rows — skipping motion-only run", len(df_mo))
    _datasets.append(("", features_df))  # full data (unfiltered comparison)

    for _suffix, _df in _datasets:
        _label = "motion-only (scoop+stab)" if _suffix == "_mo" else "all segments"
        logger.info("  === Classification: %s, %d rows ===", _label, len(_df))

        feat_cols = _feature_cols(_df)
        X = _df[feat_cols].copy().replace([np.inf, -np.inf], np.nan).fillna(0.0)
        y = _df["is_et"].astype(int).copy()
        groups = _df["patient_id"].copy()

        if len(X) < 5:
            logger.warning("  Only %d samples — skipping", len(X))
            continue
        ...
        models: Dict[str, Any] = {
            ...
            "SVC": SVC(
                kernel="rbf", probability=True, class_weight=svc_weight, random_state=cfg.RANDOM_STATE),
            "RandomForest": RandomForestClassifier(
                n_estimators=100, class_weight="balanced", random_state=cfg.RANDOM_STATE),
        ...
        for name, model in models.items():
            ...
            cv_splitter = (
                cv.split(X, y, groups) if hasattr(cv, "split")
                else StratifiedKFold(cfg.CV_FOLDS, shuffle=True,
            ...
            for train_idx, test_idx in cv_splitter:
                ...
                # Log feature names from the first fold of the first dataset
                if not _logged_features:
                    vt_cols = X.columns[vt.get_support()]
                    sel_cols = list(vt_cols[rfe.support_])
                    logger.info("RFE (1st fold, %s) selected %d features: %s",
            ...
            results[name] = metrics
            logger.info("  %-20s  %s", name,
                        "  ".join(f"{k}={v:.3f}" for k, v in metrics.items()))

            if has_proba:
                patient_metrics = compute_patient_metrics(y.values, y_score, groups.values)
                results[f"{name}_patient"] = patient_metrics
                logger.info("  %-20s  AUC=%.3f (median)", f"{name}_patient",
                            patient_metrics.get("AUC", 0))

        # Stacking (only if enabled)
        if cfg.USE_STACKING and len(models) >= 2:
            ...
            for train_idx, test_idx in cv_splitter:
                ...
                vt_ = VarianceThreshold(); sc_ = StandardScaler()
                rfe_ = RFE(
                    estimator=RandomForestClassifier(
                        n_estimators=50, max_depth=5, class_weight="balanced",
                        random_state=cfg.RANDOM_STATE, n_jobs=-1),
                ...
                X_tr_s = sc_.fit_transform(vt_.fit_transform(X_tr))
                X_te_s = sc_.transform(vt_.transform(X_te))
            ...
            st_m = compute_classification_metrics(y, y_pred_stack, y_score_stack)
            results["StackingEnsemble"] = st_m
            logger.info("  %-20s  %s", "StackingEnsemble",
                        "  ".join(f"{k}={v:.3f}" for k, v in st_m.items()))
            pm = compute_patient_metrics(y.values, y_score_stack, groups.values)
            results["StackingEnsemble_patient"] = pm
        ...
        for name, metrics in list(results.items()):
            ...
            try:
                y_score_j = cross_val_predict(
                    pipe_j, X, y, cv=cv, groups=groups, method="predict_proba")[:, 1]
            except (AttributeError, ValueError):
                continue
            opt_metrics = optimize_threshold_youden(y.values, y_score_j)
            results[f"{name}_Youden"] = opt_metrics
            logger.info("  %-20s  %s", f"{name}_Youden",
                        "  ".join(f"{k}={v:.3f}" for k, v in opt_metrics.items()))

        # Merge into all_results with suffix
        for k, v in results.items():
            all_results[k + _suffix] = v
...
# ── Bucketed regression (Scoop / Stab — hand-agnostic) ───────────────────


def run_bucketed_regression(
    features_df: pd.DataFrame,
) -> Dict[str, Any]:
    """Per-bucket LOSO CV regression: 2 hand-agnostic buckets (scoop, stab).

    Target per cycle = the CRF cell matching the hand that performed it:
        scoop cycle, Right hand → rt_scoop
        scoop cycle, Left  hand → lf_scoop  (same logic for stab)

    Extra feature added: ``is_dominant_hand`` — 1 if the performing hand
    matches the patient's CRF tremor_hand (bilateral → always 1).

    RFE is refitted independently inside each LOSO fold per bucket.
    Feature names are logged from the first fold (no full-data RFE).
    """
    from sklearn.pipeline import Pipeline
    from sklearn.base import clone
    ...
    if missing:
        logger.warning("run_bucketed_regression: missing columns %s — skipping", missing)
        return {}

    # Add is_dominant_hand feature (patient-level, no leakage)
    crf = _get_crf_data()
    def _is_dominant(row: pd.Series) -> float:
        rec = crf.get(str(row["patient_id"]), {})
        th = rec.get("tremor_hand", "")
        if th == "Bilateral":
            return 1.0
        return 1.0 if row["hand"] == th else 0.0
    et_df["is_dominant_hand"] = et_df.apply(_is_dominant, axis=1)

    results: Dict[str, Any] = {}
    feat_cols = _feature_cols(et_df) + ["is_dominant_hand"]
    # Deduplicate in case is_dominant_hand is already in _feature_cols
    feat_cols = list(dict.fromkeys(feat_cols))

    for movement_type in ["scoop", "stab"]:
        bucket_df = et_df[et_df["movement_type"] == movement_type].copy()

        if len(bucket_df) == 0:
            logger.info("Bucket %s: no samples", movement_type)
            continue

        # Per-row target: score for the hand that performed this cycle
        if movement_type == "scoop":
            bucket_df["_bucket_target"] = np.where(
                bucket_df["hand"] == "Right", bucket_df["rt_scoop"], bucket_df["lf_scoop"]
            )
        else:
            bucket_df["_bucket_target"] = np.where(
                bucket_df["hand"] == "Right", bucket_df["rt_stab"], bucket_df["lf_stab"]
            )
        target_col = "_bucket_target"

        n_patients = bucket_df["patient_id"].nunique()
        logger.info(
            "Bucket %s: %d cycles from %d patients "
            "(Right=%d, Left=%d, hand-agnostic target)",
            movement_type, len(bucket_df), n_patients,
            int((bucket_df["hand"] == "Right").sum()),
            int((bucket_df["hand"] == "Left").sum()),
        )

        if n_patients < cfg.MIN_BUCKET_PATIENTS:
            logger.warning(
                "Bucket %s: %d patients < MIN_BUCKET_PATIENTS=%d — skipping",
                movement_type, n_patients, cfg.MIN_BUCKET_PATIENTS,
            )
            results[movement_type] = {"status": f"sparse ({n_patients} patients)"}
            continue
        ...
        if len(X) < cfg.MIN_BUCKET_PATIENTS:
            logger.warning("Bucket %s: only %d valid samples — skipping", movement_type, len(X))
            continue
        ...
        for name, estimator in models.items():
            pipe = Pipeline([
                ...
                ("rfe", RFE(
                    estimator=RandomForestRegressor(
                        ...
                        random_state=cfg.RANDOM_STATE, n_jobs=-1),
                    n_features_to_select=n_select, step=0.1,
                )),
                ("model", estimator),
            ])
                ...
                for train_idx, test_idx in cv_splitter:
                    ...
                    if not _logged_features:
                        ...
                        if vt is not None and rfe is not None:
                            ...
                            sel_cols = list(vt_cols[rfe.support_])
                            logger.info(
                                "Bucket %s RFE (1st fold) → %d features: %s",
                                movement_type, len(sel_cols), sel_cols,
                            )
                            results[f"{movement_type}_features"] = sel_cols
                        _logged_features = True

                    y_pred[test_idx] = fold_pipe.predict(X.iloc[test_idx])
            except Exception as exc:
                logger.warning("Bucket %s model %s failed: %s", movement_type, name, exc)
                continue
            ...
            logger.info(
                "  Bucket %-8s %-20s MAE=%.3f R²=%.3f Pearson_r=%.3f",
                movement_type, name,
                metrics["MAE"], metrics["R2"], metrics.get("Pearson_r", 0),
            )

        results[movement_type] = bucket_results

    return results


# ── Severity classification (ET only, multi-class) ───────────────────────

def run_severity_classification(
    features_df: pd.DataFrame,
    score_col: str = "local_score",
) -> Dict[str, Any]:
    """Multi-class severity classification for ET patients via LOSO CV.

    Bins *score_col* into Mild / Moderate / Severe using thresholds from
    config (SEVERITY_LOCAL_* or SEVERITY_GLOBAL_*) and runs a multi-class
    RandomForest with VT → Scale → RFE preprocessing inside each CV fold.

    Returns a dict with ``y_true``, ``y_pred``, ``labels``, ``Accuracy``,
    and ``class_counts``; or an empty dict if classification is not feasible.
    """
    from sklearn.pipeline import Pipeline
    from sklearn.base import clone
    ...
    if len(et_df) < 6:
        logger.warning(
            "Severity classification (%s): only %d ET rows — skipping",
            score_col, len(et_df),
        )
        return {}

    if score_col == "local_score":
        bins           = cfg.SEVERITY_LOCAL_BINS
        ordered_labels = cfg.SEVERITY_LOCAL_LABELS
    else:
        bins           = cfg.SEVERITY_GLOBAL_BINS
        ordered_labels = cfg.SEVERITY_GLOBAL_LABELS

    try:
        # Use pd.Series so pd.cut returns a proper Series with .notna()
        scores_series = pd.Series(
            et_df[score_col].values.astype(float), dtype=float
        )
        severity_series = pd.cut(
            scores_series, bins=bins, labels=ordered_labels,
            right=True, include_lowest=True,
        )
        valid_mask = severity_series.notna().values          # numpy bool array
        et_df      = et_df[valid_mask].reset_index(drop=True)
        severity   = severity_series[valid_mask].astype(str).values

        class_counts   = dict(Counter(severity))
        present_labels = [l for l in ordered_labels if class_counts.get(l, 0) > 0]
        logger.info("Severity distribution (%s): %s", score_col, class_counts)

        if len(present_labels) < 2:
            logger.warning(
                "Severity classification (%s): fewer than 2 classes — skipping",
                score_col,
            )
        ...
        le    = LabelEncoder()
        y_enc = le.fit_transform(severity)
        ...
        y_pred_enc = cross_val_predict(
            pipe, X.values, y_enc, cv=cv, groups=groups,
        )

    except Exception as exc:
        logger.warning(
            "Severity classification (%s) failed: %s", score_col, exc, exc_info=True
        )
        return {}

    y_pred = le.inverse_transform(y_pred_enc)
    acc    = accuracy_score(severity, y_pred)
    logger.info(
        "Severity classification (%s): Accuracy=%.3f, classes=%s",
        score_col, acc, present_labels,
    )
